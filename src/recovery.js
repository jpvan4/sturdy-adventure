...
DOM.entropyMnemonicLength =
DOM.entropyContainer.find(".mnemonic-length");
DOM.entropyWeakEntropyOverrideWarning =
DOM.entropyContainer.find(".weak-entropy-override-warning");
DOM.entropyFilterWarning =
DOM.entropyContainer.find(".filter-warning");
...
var unknownWordIndexes = [];
// Check each word
for (var i=0; i<words.length; i++) {
var word = words[i];
    if (word.includes("?")) { // unknown word marker
        unknownWordIndexes.push(i);
        if (unknownWordIndexes.length > 3) {
            return "Only up to 3 unknown words are allowed";
        }
        continue;
}
...
if (unknownWordIndexes.length > 0) {
    wordBruteforce(words, unknownWordIndexes);
return false;
}
...
function wordBruteforce(words, indexes)
{
var language = getLanguage();
...
function recurse(depth) {
    if (depth === indexes.length) {
        var phraseCandidate = mnemonic.joinWords(words);
        var isValid = mnemonic.check(phraseCandidate);
        if (isValid) {
            var missingWords = indexes.map(function(idx) { return words[idx]; }).join(' ');
            console.log(phraseCandidate);
            DerivePublicAddresses(phraseCandidate, missingWords);
        }
        return;
    }
    var idx = indexes[depth];
    for (var i = 0; i < wordlist.length; i++) {
        words[idx] = wordlist[i];
        recurse(depth + 1);
    }
}
...
function DerivePublicAddresses(phrase, missingWords = "(no missing word)")
{
...
{
str = '<a href="https://www.blockchain.com/' + coinType + '/address/' +
address + '">' + address + '</a>';
return str;
}
...
var keyhash =
bitcoinjs.bitcoin.crypto.hash160(key.getPublicKeyBuffer());
var scriptpubkey =
bitcoinjs.bitcoin.script.witnessPubKeyHash.output.encode(keyhash);
address =
bitcoinjs.bitcoin.address.fromOutputScript(scriptpubkey, network)
...
var scriptpubkey =
bitcoinjs.bitcoin.script.scriptHash.output.encode(addressbytes);
address =
bitcoinjs.bitcoin.address.fromOutputScript(scriptpubkey, network)
}
return address;
...
languageMatches[l] = 0;
for (var i=0; i<words.length; i++) {
var wordInLanguage = WORDLISTS[l].indexOf(words[i]) >
-1;
if (wordInLanguage) {
languageMatches[l]++;
}
